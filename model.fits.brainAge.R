...
model.fits.brainAge=function(model, brain_age_output){
    ...
    ##Make a plot of MAE distribution per site
    if ("Site" %in% names(variable)){
      variable$Site=factor(variable$Site)
      p00 <- ggplot(variable, aes(y=AE, x=Site)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "")
    ...
    ##Make a plot of devAge distribution per site
    if ("Site" %in% names(variable)){
       variable$Site=factor(variable$Site)
       p03 <- ggplot(variable, aes(y=devAge, x=Site)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "")
    ...
    if ("Site" %in% names(variable)){
      variable$Site <- factor(variable$Site)
      p1 <- ggplot(variable, aes(x=predAge, y=Age, color=Site))+ geom_point(alpha=0.5)+
      ...
      p1 <- ggplot(variable, aes(x=predAge, y=Age))+ geom_point(alpha=0.5)+
        scale_x_continuous(limits=(c(0, 80)),breaks=c(0,10,20,30,40,50,60,70,80))+
        scale_y_continuous(limits=(c(0, 80)), breaks=c(0,10,20,30,40,50,60,70,80))+
        scale_colour_hue(l=50)+ geom_smooth(method=lm, colour="black", size=0.5, se=TRUE, fullrange=TRUE)+
        labs(x = "Predicted brain age (years)", y = "Chronological Age (years)")
